...
public class RatingBar extends View {
	...
	private boolean integerMark = false;// 整型评价
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (starEmptyDrawable == null) {
			starEmptyDrawable = getResources().getDrawable(R.drawable.xing_hui);
		}
		Drawable drawable = mTypedArray.getDrawable(R.styleable.RatingBar_starFill);
		if (drawable == null) {
			drawable = getResources().getDrawable(R.drawable.xing);
		}
		this.starFillBitmap = drawableToBitmap(drawable);
		isCanClick = mTypedArray.getBoolean(R.styleable.RatingBar_clickAble, true);
		integerMark = mTypedArray.getBoolean(R.styleable.RatingBar_integerMark, false);
		mTypedArray.recycle();
	...
	public void setIsClick(boolean isCanClick) {
	 ...
	 * @param mark
	 */
	public void setStarMark(float mark, boolean formUser) {
		if (integerMark) {
			...
			if ((mark * 10) % 5 == 0) {
				...
				starMark = meg(Math.round(mark * 10) * 1.0f / 10);
//				if ((mark * 10) / 5 > 2.5) {
//				} else {
//					starMark = (float) (meg(mark)+0.5);
//				}
			}
		}
		if (this.onStarChangeListener != null) {
			this.onStarChangeListener.onStarChange(starMark, formUser); // 调用监听接口
		}
		invalidate();
	}

	private float meg(float i) {
		int a=(int) Math.ceil(i);
		float c;
		if ((float)a-i>0.5) {
			c=(float)(a-0.5);
		}else {
			if (a==(int)Math.ceil(i+0.5)) {
				c=(float)(a-0.5);
			}else {
				c=(float)a;
			}
	...
	void dispatchRatingChange(boolean fromUser) {
		if (onStarChangeListener != null) {
			onStarChangeListener.onStarChange(starMark, fromUser);
		}
	}
	...
	public interface OnStarChangeListener {
		void onStarChange(float mark, boolean formUser);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < starCount; i++) {
			starEmptyDrawable.setBounds((starDistance + starSize) * i, 0, (starDistance + starSize) * i + starSize,
					starSize);
		...
		if (starMark > 1) {
			...
			if (starMark - (int) (starMark) == 0) {
				...
				canvas.translate(starDistance + starSize, 0);
				canvas.drawRect(0, 0, starSize * (Math.round((starMark - (int) (starMark)) * 10) * 1.0f / 10), starSize,
						paint);
			}
		} else {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN: {
			setStarMark(x * 1.0f / (getMeasuredWidth() * 1.0f / starCount), true);
			break;
		}
