...
import allen.frame.widget.GestureLockView.Mode;

public class GestureLockViewGrop extends RelativeLayout {
	...
	public GestureLockViewGrop(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public GestureLockViewGrop(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mGestureLockViews == null) {
			...
			for (int i = 0; i < mGestureLockViews.length; i++) {
				...
				LayoutParams lockerParams = new LayoutParams(mGestureLockViewWidth,
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mOnGestureLockViewListener != null) {
				mOnGestureLockViewListener.onGestureBegin();
			}
			break;
		...
		case MotionEvent.ACTION_UP:
			...
			// 计算每个元素中箭头需要旋转的角度
			/*
			 * for (int i = 0; i + 1 < mChoose.size(); i++) { int childId =
			 * mChoose.get(i); int nextChildId = mChoose.get(i + 1);
			 * 
			 * GestureLockView startChild = (GestureLockView)
			 * findViewById(childId); GestureLockView nextChild =
			 * (GestureLockView) findViewById(nextChildId);
			 * 
			 * int dx = nextChild.getLeft() - startChild.getLeft(); int dy =
			 * nextChild.getTop() - startChild.getTop(); // 计算角度 int angle =
			 * (int) Math.toDegrees(Math.atan2(dy, dx)) + 90;
	 ...
	 * @param isRight
	 */
	private void setViewClor(boolean isRight) {
		if (isRight) {
