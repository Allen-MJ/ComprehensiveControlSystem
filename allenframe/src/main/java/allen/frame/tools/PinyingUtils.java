...
public class PinyingUtils {
	 ...
	 * @param DYZTag 是否支持多音字
	 * @return
	 */
	public static char[]  getHeadByChar(char src,boolean isCapital,boolean DYZTag){
		//如果不是汉字直接返回
		if (src <= 128) {
			return new char[]{src};
		}
	...
	public static char[]  getHeadByChar(char src){
		return getHeadByChar(src,false,false);
	}
	/**
	 * 查找字符串首字母
	 * @param src 
	...
	public  static String[] getHeadByString(String src,boolean isCapital,String separator){
		...
		for(char ch:chars){
			...
			char[]chs=getHeadByChar(ch,isCapital,true);
			StringBuffer sb=new StringBuffer();
			if(null!=separator){
				int j=1;
