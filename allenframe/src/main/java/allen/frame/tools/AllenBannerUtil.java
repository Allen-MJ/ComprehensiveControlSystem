 ...
 * 描述:
 */
public class AllenBannerUtil {

    private AllenBannerUtil() {
    }
    ...
    public static LoadBitmapPair<Throwable> getImageFromResource(@NonNull Context context, int resId, int maxWidth, int maxHeight) {
        ...
        try {
            if (maxWidth == 0 && maxHeight == 0) {
                ...
                result = new LoadBitmapPair<Throwable>(BitmapFactory.decodeStream(is, null, decodeOptions), null);
                is.close();
            } else {
                ...
                if (tempBitmap != null && (tempBitmap.getWidth() > desiredWidth || tempBitmap.getHeight() > desiredHeight)) {
                    result = new LoadBitmapPair<Throwable>(Bitmap.createScaledBitmap(tempBitmap, desiredWidth, desiredHeight, true), null);
                    tempBitmap.recycle();
                } else {
                    result = new LoadBitmapPair<Throwable>(tempBitmap, null);
                }
            }
