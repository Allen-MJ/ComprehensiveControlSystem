...
public class EventMonthActivity extends AllenBaseActivity {
    ...
    @BindView(R2.id.choice_month)
    AppCompatTextView choiceMonth;
    ...
    protected int getLayoutResID() {
        return R.layout.core_event_month_layout;
    }
    ...
    protected void initUI(@Nullable Bundle savedInstanceState) {
        ...
        String text = year + "-" + String.format("%02d", monthOfYear + 1);
        choiceMonth.setText(text);
        initChart();
        loadData();
    }
...
//        不显示X轴 Y轴线条
    ...
    @OnClick({R2.id.choice_month, R2.id.choice_dw})
    public void onViewClicked(View view) {
        view.setEnabled(false);
        int id = view.getId();
        if (id == R.id.choice_month) {
            Calendar calendar = Calendar.getInstance();
            DatePickerDialog dialog = new DatePickerDialog(context, new DatePickerDialog.OnDateSetListener() {
                @Override
                public void onDateSet(DatePicker startDatePicker, int startYear, int startMonthOfYear, int startDayOfMonth) {
                    choiceMonth.setText(startYear + "-" + String.format("%02d", startMonthOfYear + 1));
                    loadData();
                }
    ...
    private void loadData() {
        Https.with(this).url(CoreApi.CoreEventsMonth)
                .addParam("month", choiceMonth.getText().toString())
